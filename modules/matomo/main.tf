...
locals {
  ...
  matomo_script        = "~/install_matomo.sh"
  security_script      = "~/configure_local_security.sh"
  create_matomo_db     = "~/create_matomo_db.sh"
...
data "template_file" "create_matomo_db" {
  template = file("${path.module}/scripts/create_matomo_db.sh")
...
resource "oci_core_instance" "Matomo" {
  count               = var.nb_of_webserver
  ...
  provisioner "file" {
    ...
    destination = local.matomo_script
 ...
 provisioner "file" {
    content     = data.template_file.create_matomo_db[count.index].rendered
    destination = local.create_matomo_db
   ...
   provisioner "remote-exec" {
    ...
    inline = [
       ...
       "chmod +x ${local.matomo_script}",
       "sudo ${local.matomo_script}",
       "chmod +x ${local.security_script}",
       "sudo ${local.security_script}",
       "chmod +x ${local.create_matomo_db}",
       "sudo ${local.create_matomo_db}"
    ]
